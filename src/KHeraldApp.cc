...
void KHeraldApp::initialize(int stage)
{
    if (stage == 0) {
        ...
        // - introduce popularity of messages.
        ...
        // - The final values (only 0 and 100s possible) are stored and used later at simulation time
        ...
        messageDeliveryProbabilitySignal = registerSignal("messageDeliveryProbabilitySignalComp");
    ...
    if (msg->isSelfMessage() && msg->getKind() == KHERALDAPP_REGISTRATION_EVENT) {
        ...
        // compute stats
         if (timesMessagesReceived[i] > 1) {
            ...
            emit(messageDeliveryProbabilitySignal, dataMsg->getProbabilityOfDelivery());
        }
