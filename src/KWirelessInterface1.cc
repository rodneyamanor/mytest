...
void KWirelessInterface1::initialize(int stage)
{
    if (stage == 0) {
        ...
        advertisementInterval = par("advertisementInterval"); //I added
        //advertisementDelay = &par("advertisementDelay"); //I added
        beaconTime = par("beaconTime"); //I added, initialize Tbeacon
        //beaconTime = uniform(0,advertisementInterval,4);
        //randomDelay = &par("randomDelay"); //I added
        usedRNG = par("usedRNG");
        timingDelay = par("timingDelay");
        //EV<<"Advertisement Delay is "<<advertisementDelay->doubleValue()<<", beacon time difference is "<<beaconTime<<", randomDelay is "<<randomDelay->doubleValue()<<"\n";
        ...
        ownNodeInfo->beaconTimeDifference = this->beaconTime; //get Tbeacon
        ...
        for (int id = 0; id <= getSimulation()->getLastComponentId(); id++) {
            ...
            if (unknownMobilityModule != NULL && unknownWirelessIfcModule != NULL) {
                ...
                double otherNodeTime = nodeInfo->nodeWirelessIfcModule->par("beaconTime").doubleValue(); // Tbeacon of other nodes
                double timeDifference = this->beaconTime - otherNodeTime; //find Tbeacon1-Tbeacn2
                nodeInfo->beaconTimeDifference = abs(timeDifference);   //store time difference between station pairs
        ...
        //set up beacon wait time
        sendBeaconEvent = new cMessage("Send Advertisement Event");
        sendBeaconEvent->setKind(KWIRELESSINTERFACE1_BEACON_EVENT_CODE);
        ...
        probabilityOfSuccess = registerSignal("linkPacketProbability"); //I added
        totalBeaconDelayTime = registerSignal("delay");
        probabilityofSuccessCountSignal =registerSignal("linkPacketProbabilityCount");
        probabilityNonSuccId = registerSignal("linkNonSuccIdProb"); //I added
...
void KWirelessInterface1::handleMessage(cMessage *msg)
{
   ...
   //Send Beacon after the delay
   //else if(msg->isSelfMessage() && msg->getKind()==KWIRELESSINTERFACE1_BEACON_EVENT_CODE) {
        /*AdvertisementMsg *advertPacket = new AdvertisementMsg("Advert Packet");
        advertPacket->setSourceAddress(ownMACAddress.c_str());
        advertPacket->setByteLength(16);
        advertPacket->setBeaconTime(simTime());
        AdvertisementMsg *copyAdvertPacket = advertPacket->dup(); */


        /*if (sendPacketTimeoutEvent->isScheduled()) {
            //packetQueue.push(copyAdvertPacket);
            packetQueue.push(upperMessage);
        }

        else {
            //setupSendingMsg(copyAdvertPacket);
            setupSendingMsg(upperMessage);
        }

        //If Queue is not empty setup next message to be sent
        //I added
        if(!beaconQueue.empty()) {
            upperMessage =beaconQueue.front();
            beaconQueue.pop();
            tRand = randomDelay->doubleValue();
            tAdv = advertisementDelay->doubleValue();
            totalDelay= tRand + tAdv;
            scheduleAt(simTime()+totalDelay,sendBeaconEvent);
        }*/
    ...
    // process a packet (arriving from upper or lower layers)
       ...
       // get message arrival gate name
        ...
        if (strstr(gateName, "upperLayerIn") != NULL) {
            ...
            //I added
           /* if(sendBeaconEvent->isScheduled()) {

                beaconQueue.push(msg);
            }

            else {
                upperMessage = new cMessage;
                upperMessage = msg;
                tRand = randomDelay->doubleValue();
                tAdv = advertisementDelay->doubleValue();
                totalDelay= tRand + tAdv;
        ...
        // from lowerLayerIn
            ...
            /*if(sendBeaconEvent->isScheduled()) {
                simtime_t beaconTimeOne = sendBeaconEvent->getSendingTime();
                simtime_t beaconTimeTwo = msg->getSendingTime();
                beaconDifference = beaconTimeOne-beaconTimeTwo;
            }

            else {
                beaconDifference = msg->getSendingTime();
...
void KWirelessInterface1::setupSendingMsg(cMessage *msg)
{
    ...
    //I added a few lines
    tRand = uniform(0,timingDelay,usedRNG); //randomDelay->doubleValue();
    tAdv = uniform(0,advertisementInterval,usedRNG); //advertisementDelay->doubleValue();
    totalDelay= tRand + tAdv;
    overlapTime = txDuration;
    //EV<<"random: "<<tRand<<"advert: "<<tAdv<<"total: "<<totalDelay<<"\n";
    EV<<"TX Duration: "<<txDuration<<"\n";
    scheduleAt(simTime() + txDuration + totalDelay, sendPacketTimeoutEvent);
...
void KWirelessInterface1::sendPendingMsg()
{
    ...
    int neigh_count = 0; // check the number of neighbours in the vulnerable window
    ...
    while (iteratorAtTxNeighbourNodeInfo != atTxNeighbourNodeInfoList.end()) {
        ...
        while (iteratorCurrentNeighbourNodeInfo != currentNeighbourNodeInfoList.end()) {
            ...
            if (atTxNeighbourNodeAddress == currentNeighbourNodeAddress) {
                ...
                EV<<"beaconTimeDifference: "<<currentNeighbourNodeInfo->beaconTimeDifference<<"\n";

                if(currentNeighbourNodeInfo->beaconTimeDifference < overlapTime) {
                    neigh_count += 1; // If the neighbor is in the vulnerable window, increase the count
                    // probability =1-(2*overlapTime/timingDelay);

                    // probNonId = probability + ((1-probability)*(currentNeighbourNodeInfo->beaconTimeDifference)/timingDelay);
                }
                ...
                //emit(totalBeaconDelayTime,totalDelay);
                //emit(probabilityNonSuccId,probNonId);
                //emit(totalBeaconDelayTime,totalDelay);
                //emit(probabilityOfSuccess, probability);

                /*if(dynamic_cast<KDataMsg*>(currentPendingMsg) != NULL) {
                    KDataMsg *dataMsg = dynamic_cast<KDataMsg*>(currentPendingMsg);
                    dataMsg->setProbabilityOfDelivery(dataMsg->getProbabilityOfDelivery()*probNonId);
                    dataMsg = nullptr;
                }*/
    ...
    if(dynamic_cast<KDataMsg*>(currentPendingMsg) != NULL) {
        double avg_tx_attempts = (neigh_count * overlapTime) / (advertisementInterval + timingDelay/2);
        probNonId = exp(-2*avg_tx_attempts);
        emit(probabilityNonSuccId,probNonId);
        emit(totalBeaconDelayTime,totalDelay);
    }
...
string KWirelessInterface1::getDestinationAddress(cMessage *msg)
{
    ...
    /* I Added
    AdvertisementMsg *advertisementMsg = dynamic_cast<AdvertisementMsg*>(msg);
    if (advertisementMsg) {
        return advertisementMsg->getDestinationAddress();
    } */
...
void KWirelessInterface1::finish()
{
    ...
    while(!beaconQueue.empty()) {
        cMessage *nextMsg = beaconQueue.front();
        beaconQueue.pop();
        delete nextMsg;
    }
